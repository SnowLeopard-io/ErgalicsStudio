...
![Ergalics Studio — 标准模式（拖入 → 即可见）](docs/Estudio.png)
...
- 宿主管理的 **Three.js 3D 场景**（`Scene3DHandle`）：网格/坐标轴/灯光、轨道控制、缩放处理、自动相机适配，以及 GPU 安全的销毁。3D 表面按需懒创建——仅对声明了 3D 能力（`renderToScene`）的插件创建——且**当 2D 插件激活时自动隐藏**，因此 3D 坐标系绝不会渗透到 2D 视图中。所有 3D 插件共享这一个宿主场景，每次切换插件都会完整清理上一个插件的对象——即便在多个 3D 插件之间直接跳转，视图之间也不会有任何残留。

**插件系统**

- **59 个内置插件**——49 个核心/科学插件外加 10 个趣味与工具玩具——覆盖完整 API 面（2D canvas、Three.js 场景、WGSL 计算、按钮/开关、沙箱、浏览器内模型训练）。
...
- **Worker 优先的插件隔离**（§6.2）：第三方入口代码运行于 Web Worker 内，通过 postMessage RPC 桥接——无法访问宿主页面的全局变量、DOM 或 stores。Canvas 渲染通过转移的 `OffscreenCanvas` 完成。当 Worker 不可用时回退到主线程上尽力而为的 `new Function` 受限作用域——该回退**不是安全沙箱**（存在可被刻意绕过的逃逸路径），且与其他 `.cspkg` 加载一样受 Ed25519 签名门禁约束：篡改包必拒，未签名/未知密钥包需用户显式信任确认（FR-05）。精确信任边界见 `SECURITY.md`。
- 支持本地化的参数面板（范围 / 选择 / 数字 / 复选框 / 文本 / 文件 / 按钮 / 开关）。
- 每个内置插件均支持一键导出：2D canvas 或 3D 场景快照导出为 PNG；凡持有表格数据的图表/查看器均可导出 RFC-4180 CSV（UTF-8 BOM，仿真类带行数上限与抽样）。多个插件还新增了分析叠加层——OLS 趋势线与移动均值、累积/密度直方图、箱线图均值标记、小提琴抖动点、柱条排序，以及生命游戏图案预设。地理插件系列的每个插件还额外提供**发送到 Figure Studio**动作，一键组装多面板图组——自动面板标签 + 中英双语的方法与公式图注。
...
| 工具 | 核心 | 亮点 |
| ---- | ---- | ---- |
| 实验记录 | `src/core/experiment/` | Flow / 积木 / 代码 / Notebook / 参数扫描 / 不确定性 / 模型实验室的每次运行都记录进项目级 IndexedDB 存储，含来源、参数、指标与耗时；任意两次运行参数并排对比 |
| GPU 不确定性引擎 | `src/core/uncertainty/` | bootstrap 置信区间、蒙特卡洛误差传播、Metropolis–Hastings MCMC；WGSL GPU 路径（PCG32 随机数、每链一个 workgroup）加速百万级重采样；R-hat / ESS 收敛诊断 |
| 模型实验室 | `src/core/model/` | OLS（QR）、逻辑回归（IRLS）、岭回归（K 折交叉验证）、多项式回归；系数表（估计 / SE / p / CI）+ 2×2 残差诊断图 |
| 数据画像 | `src/core/profiler/` | 单遍流式扫描产出逐列画像、相关矩阵、0–100 质量评分与问题清单；按内容指纹缓存，二次打开秒出 |
| 信号实验室 | `src/core/signal/` | FFT / Welch PSD、窗函数、Savitzky–Golay 与移动平均滤波、ACF/PACF、季节分解；滤波结果可另存为派生文件并自动接入血缘 DAG |
| 参数扫描 | `src/core/sweep/` | 1–3 个参数轴（网格 / 列表 / 拉丁超立方）批量运行任意管线；误差棒折线、响应面热力图、平行坐标；计划草稿经纯领域层逐字段校验，自动检测过期结果 |
| SQL 工作台 | `src/core/sql/` | 懒加载 DuckDB-WASM 将项目文件注册为表；Monaco 中 join / 聚合 / 窗口函数查询；结果保存为新 CSV 并继承血缘边 |
| 报告生成器 | `src/core/report/` | 按序组合各节（标题、Markdown、图表、表格、运行摘要、交互筛选器）→ 单个自包含 HTML：内联 SVG、原生 JS 交互、亮/暗主题、中英双语 |
| 可复现锁 | `src/core/repro/lock.ts` | 导出 `repro.lock`（数据指纹 + 参数哈希 + 种子 + 代码快照），五类漂移校验，一键重跑确认指标可复现 |
| 单位系统 | `src/core/units/` | 带 SI 词头解析、量纲代数与换算检查的类型化 `Quantity`；`units.convert` / `units.check` 流程区块 + `QuantityInput` 控件 |
| 数据血缘 | `src/core/lineage/` | 由运行记录与导入事件自动重建的文件→运行 DAG，分层 SVG 画布；SQL 查询、派生列与参数扫描均作为节点出现 |
| 分块读取 | `src/core/chunked/` | 面向大型分隔符文件的异步行窗口读取器，支持列投影、预览抽样与内容指纹 |
| Figure Studio | `src/core/figure/` | IEEE / Elsevier 模板上的多面板出版级图表，自动面板标签、图注、实时 SVG 预览与 SVG / PDF / PNG-600dpi 导出 |
| 补充材料打包 | `src/core/package/` | 一键构建论文随附 ZIP：`manifest.json`（元数据 + 运行记录 + 血缘图 + 作者/许可/描述表单）+ 可选数据文件与代码会话 |
| Notebook | `src/core/notebook/` | Markdown/代码混合单元格持久化于项目内；代码单元格运行于专用 Pyodide 运行时，每次运行汇入实验历史 |
| 推断引擎 | `src/core/inference/` | HMC / NUTS 采样器（DualAveraging、U-turn），R-hat / bulk & tail-ESS、HDI、MCSE、WAIC / PSIS-LOO、PPC；声明式似然模板配数据尺度弱信息先验，无需写代码即可拟合 |
| 分析页 | `/#/analysis` | 快速路径：选择数据文件即得折线 / 散点 / 直方图 / 柱状图、描述统计与单/双样本 / Mann–Whitney 检验，支持 SVG / PDF 导出 |
...
![标准模式 — 拖入文件，即可看到可视化](docs/Estudio.png)
...
在 Figure Studio 之上是一条出版级流水线。**期刊投稿检查**会对当前图版按 **IEEE / Elsevier** 规范做预检，按 *image / annotation / text / metadata* 分组核对：有效导出分辨率（600 / 300 dpi）、RGB 与 CMYK 色彩模式、矢量输出的字体嵌入、小写图板标签、轴标签、配文图注、可接受的导出格式、≥ 7 pt 的标签字号，以及匹配的期刊模板。每一项未通过都提供"定位问题"动作，可直达对应控件；每种导出格式提交前也会显示未通过项计数。**图注起草器**把图表类型、列名与可选的统计结果转成可编辑的"图 1. ……"图注（英文 / 中文），图板还可附带**元数据标注**，这些都会同时进入投稿检查与补充材料 manifest。面板图型现已包含 **3D 场图**：在真实 3D 盒子中绘制的着色曲面——底部网格、曲面四边形与 z 轴——由同一套 SVG 引擎渲染。
...
| 太阳高度与昼长（Solar Elevation & Day Length） | `.json`（地点 + 年份） | 全年正午太阳高度、赤纬与昼长；FAO-56 日辐射估算（地外辐射 R_a eq. 21 + 晴空辐射 R_so eq. 37） |
| 气候图（Climatograph） | `.csv`, `.txt`（月, 温度, 降水） | 温度折线 + 降水柱状气候图，附柯本（Köppen）风格分类摘要 |
| 人口金字塔（Population Pyramid） | `.csv`（年龄 × 性别） | 背靠背年龄-性别金字塔，附抚养比与增长形态解读 |
| 空间插值（Spatial Interpolation） | `.csv`（站点 x, y, 值） | IDW + 普通克里金（自动拟合变异函数）网格化与等值线；LOOCV（RMSE/MAE）留一交叉验证 + 全局 Moran's I 空间自相关（随机化检验 z 分数） |
| 测距与测面（Distance & Area Measure） | `.json`, `.csv` | 大圆测距、球面多边形面积与标准差椭圆（1σ / 2σ，闭式特征解） |
| 投影变形（Tissot 圆） | `.json` | 七种世界投影的 Tissot 指纬线与局部变形指标（h / k / 面积比） |
| 地形分析（DEM Terrain Analysis） | `.asc`（ESRI ASCII Grid） | 山体阴影 / 坡度 / 坡向 / 等高线、3D 网格视图与流域分析：priority-flood 填洼 → D8 流向 → 汇流累积 |
| GPX 轨迹分析（GPX Track Analysis） | `.gpx` | 距离、迟滞滤波爬升/下降、耗时与海拔剖面 |
| 3D 地球仪（Interactive Globe） | `.json` | 可拖拽旋转的 3D 地球仪：海岸线、经纬网、球面 Tissot 圆与自转 |
| 晶胞 · 3D 预览（Crystal · 3D Unit Cell） | `.cif`, `.poscar`, `.vasp`, `.xyz` | 晶体晶胞 3D 预览：原子、周期键、约化式与密度估计 |
| 反应 · 自由反应动力学 3D（Reaction · MD 3D） | `.json`（反应场景） | 3D 反应分子动力学（NumPy/Langevin 引擎）：化学键越过 Arrhenius 势垒断裂并重组，原子运动来自真实物理 |
| 酶动力学（Enzyme Kinetics） | `.csv`, `.tsv`, `.json`, `.dat` | 米氏方程拟合（Levenberg–Marquardt）：竞争 / 非竞争 / 反竞争抑制与 Lineweaver–Burk 线性化 |
| 传染病分室模型（Epidemic Modeling） | `.json`（配置）, `.csv` / `.tsv` / `.dat`（病例） | RK4 积分的确定性 SIR / SEIR 分室模型：R₀、群体免疫阈值、峰值时间与罹患率 |
| 序列比对与分析（Sequence Alignment） | `.fasta`, `.fa`, `.json`, `.csv`, `.tsv` | Needleman-Wunsch / Smith-Waterman 成对比对（仿射间隙）+ 组分与滑窗 GC 剖析 |
| 群体遗传学（Population Genetics） | `.csv`, `.tsv`, `.json`, `.vcf`, `.dat` | Hardy–Weinberg 平衡卡方检验 + 带种子、完全可复现的 Wright–Fisher 遗传漂变模拟（可选选择压） |
...
**地理插件系列**——除上文 GeoJSON Map 之外的九个科研级地理插件（`src/plugins/builtin/geo/`），每个都支持一键发送 Figure Studio，组装多面板图组并附中英双语的方法与公式图注。亮点：**空间插值**以 IDW 或普通克里金（自动拟合变异函数）对离散站点网格化，用留一交叉验证（RMSE / MAE）自评精度，并运行全局 Moran's I 空间自相关检验（随机化 z 分数）；**地形分析**从 ESRI ASCII Grid 渲染山体阴影 / 坡度 / 坡向 / 等高线，提供 3D 网格视图，并以完整的 priority-flood 填洼（Barnes 2014，ε 梯度）→ D8 最陡流向 → Kahn 拓扑汇流累积三件套划定流域；**测距与测面**计算大圆距离、球面多边形面积与闭式解 1σ / 2σ 标准差椭圆；**太阳高度与昼长**按 FAO-56 公式估算日辐射（地外辐射 R_a eq. 21 + 晴空辐射 R_so eq. 37）。其余五个覆盖投影变形（Tissot 指纬线与局部 h / k / 面积比指标）、柯本风格气候图、人口金字塔、GPX 轨迹（迟滞滤波爬升 + 海拔剖面）与可拖拽自转的 3D 地球仪。
...
| 领域 | 单元测试钉住的内容 |
| ---- | ------------------ |
| 数据与 I/O | 文件格式检测、科研二进制 I/O（NetCDF/HDF5/FITS/Parquet/Zarr 辅助）、分块读取 |
| 统计内核 | 描述统计、特殊函数、假设检验、效应量、多重比较校正、功效分析 |
| 插件与沙箱 | cspkg 解析/校验、沙箱 RPC（含穿越 fake Worker 的端到端往返）、内置插件逻辑（含一键 PNG/CSV 导出动作、宿主按钮载荷与缺陷回归）、数据插件解析辅助（误差带、矩形树层级、QQ probit）、插件运行时生命周期、结构力学模拟器 |
| GPU 计算 | WGSL 模板（粒子、N-Body、直方图、热力图、点云）、缓冲打包、CPU 积分器、服务门控 |
| 区块系统 | `DataTable` ops、注册表、编译器（校验/拓扑/类型检查）、执行器（增量缓存 + 失效）、几何、目录执行器、`viz.*` → 插件渲染桥接、代码生成（JS/Python/R） |
| 三模式 IR 同步 | 积木 ↔ 流程 ↔ 代码往返（含 `mergeFlowIR`、按会话语言互译、流程签名守卫）、Python/R/JavaScript 代码解析器、`examples-roundtrip` 经 IR 解释器端到端执行全部随附 `.clproj`、studio API 流程对齐方法、Pyodide worker 协议 |
| 科研模块 | 不确定性套件（bootstrap、蒙特卡洛传播、GPU 引擎对拍、R-hat/ESS）、单位系统、实验记录（IndexedDB runs 存储）、数据血缘、图表组合、补充材料打包（zip 往返）、Notebook 模型、模型实验室、数据画像、信号工具箱、参数扫描执行器 + 草稿/响应面层、SQL 引擎、报告生成器、可复现锁、推断引擎模板（端到端采样 + WAIC/LOO/PPC + 确定性对拍）、`fmt` 边界安全 |
| 可靠性内核 | 结构化错误分类法（归一化、因果链、Result 组合子、重试/中止、注册表去重、全局处理器）、校验框架（可组合校验器、嵌套问题路径、安全解析）、数据质量引擎（类型推断、IQR 离群值、期望规则、schema 推断、坏行隔离、DataTable 适配器） |
| 平台 | i18n、app store、WASM 重试策略、出版级绘图引擎、可复现性内核、经 `import.meta.glob` 加载的管线示例 |
...
- [x] 工作台布局、项目管理、文件路由
- [x] 59 个内置插件（49 科学 + 10 趣味/工具）、cspkg 加载、Worker 沙箱
- [x] 插件导出与分析增强——59 个插件全部支持一键 PNG 快照（3D 走场景快照）与 RFC-4180 CSV 导出，另有趋势线 / 移动均值 / 累积 / 密度 / 抖动 / 排序叠加层与仿真预设（生命游戏图案、Truchet 砖型）
- [x] 地理插件系列——十个科研级地理插件（空间插值含 LOOCV + Moran's I、地形分析含 priority-flood → D8 → 汇流累积流域三件套、FAO-56 太阳辐射、Tissot 指纬线、标准差椭圆测距测面、柯本气候图、人口金字塔、GPX 轨迹、3D 地球仪），每个均支持一键发送 Figure Studio 组装多面板图组并附中英双语图注
