...
`src/core` 下除横切服务外，另有 **34 个领域子系统目录**。其中 33 个已落地，为纯 TypeScript、少 DOM、可在 Node 环境用 Vitest 直接测试；`cfd`（流体与场耦合求解器）尚在规划中，尚未提交源码。它们被区块系统、Studio API、科研工具页与交付管线共同消费，是产品从"能画图"走向"可用于真实科研"的能力层。
