...
> 本文是 Ergalics Studio 的完整技术总结，覆盖系统架构、插件体系、四种工作模式、科研工具集、GPU 计算引擎与测试质量保障，另附使用指南与文档索引。数字基线为 44 个内置插件、42 个流程区块、19 个科研工具、33 个领域子系统、111 个测试文件 / 1849 个单元测试、13 套端到端套件、5 条 CI 工作流。
...
| 电磁谐振特征值求解器 EM Eigensolver | .npz、.npy、.mtx | 稀疏厄密特征值求解器（thick-restart Lanczos / LOBPCG / Jacobi-Davidson + MINRES 位移反演），2D 谱图报告与 3D 模式场，Pyodide Worker 运行 |
...
| 物理模拟 physics | 10 | 粒子、N-Body、流体、波动方程、双摆、电磁场、光学实验、结构力学、电磁谐振特征值求解器、流体双向耦合求解器 |
| 地理 geo | 1 | GeoJSON 地图 |
...
| 03-插件系统 | 插件契约、44 个内置插件明细、市场目录与两级加载、cspkg 沙箱与 Ed25519 签名、文件路由 |
