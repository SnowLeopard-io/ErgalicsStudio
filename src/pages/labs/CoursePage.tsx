    ...
    return (
      <ToolShell toolId="course">
        <div className="analysis-body">
          <h4 className="share-section-title">{t('course.identity')}</h4>
          <p className="analysis-note">{t('course.identity_need')}</p>
          <div className="analysis-row">
            ...
            <button
              ...
              onClick={() => writeLocal(IDENTITY_KEY, identity)}
              disabled={!identity.trim()}
            >
              {t('course.identity_set')}
            </button>
          </div>
        </div>
      </ToolShell>
  ...
  return (
    <ToolShell toolId="course">
      <div className="analysis-body">
        {/* ---- identity + role ---- */}
        <div className="analysis-row">
          <span className="analysis-label">{t('course.identity_current', { name: identity })}</span>
          <input
            className="input"
            style={{ maxWidth: 180 }}
            aria-label={t('course.identity')}
            value={identity}
            onChange={(e) => {
              setIdentity(e.target.value);
              writeLocal(IDENTITY_KEY, e.target.value);
            }}
          />
          <label className="analysis-label">{t('course.role')}</label>
          <select
            className="input"
            value={role}
            onChange={(e) => {
              const next = e.target.value === 'student' ? 'student' : 'teacher';
              setRole(next);
              writeLocal(ROLE_KEY, next);
              setOpenCourseId(null);
            }}
          >
            <option value="teacher">{t('course.role.teacher')}</option>
            <option value="student">{t('course.role.student')}</option>
          </select>
        </div>
        {note && <p className="analysis-note">{note}</p>}
        {!project && <p className="analysis-note">{t('course.submit_need_project')}</p>}
        ...
        {!data ? null : !openCourse ? (
            ...
            <ul className="course-list">
              {data.courses.map((c) => (
                <li key={c.id} className="course-list-item">
                  ...
                  <span className="analysis-note">{c.assignments.length} · {c.id.slice(0, 8)}</span>
                </li>
              ))}
            </ul>
          </>
          ...
          <>
            ...
            <div className="analysis-row">
              ...
              <h4 className="share-section-title" style={{ margin: 0 }}>{openCourse.name}</h4>
            ...
            {isTeacher ? (
              <>
                ...
                <textarea className="input" rows={2} value={asgDesc} placeholder={t('course.assignment_desc')} onChange={(e) => setAsgDesc(e.target.value)} />
                  ...
                  return (
                    <div key={a.id} className="course-assignment-card">
                      <div className="analysis-row">
                        ...
                        {a.templateId && <span className="analysis-note">{getTemplate(a.templateId)?.title.en ?? a.templateId}</span>}
                        <button type="button" className="btn btn-sm" onClick={() => void captureExpectation(a.id)}>
                          {t('course.set_expectation')}
                        </button>
                        <button type="button" className="btn btn-sm" onClick={() => void withdrawAssignment(a.id)}>
                ...
                <div className="analysis-row">
                  ...
                  <button type="button" className="btn" onClick={() => void deleteCourseClick()}>{t('course.delete')}</button>
                </div>
              </>
            ) : (
              <>
