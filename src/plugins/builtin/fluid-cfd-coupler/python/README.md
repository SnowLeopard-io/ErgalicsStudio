# fluid_cfd — 1D 管网 ↔ 3D 不可压 N–S 场 双向耦合内核

`fluid_cfd` 是附属于 fluid-cfd-coupler 插件的纯 NumPy 数值内核。它将**粗时间步的
1-D 管网/喷管网络**与**细时间步的 3-D 不可压 Navier–Stokes 场求解器**通过**多速率
子循环（multi-rate sub-cycling）** 在交换窗口（exchange window）处双向耦合起来，
...
│   ├── analytic.py     # 解析解（临界流量、放气背压、亚临界文献关系）——验证基准
│   ├── network_1d.py   # 1-D 管网求解器 + 阀门控制逻辑
│   ├── domain_3d.py    # 3-D 不可压 N–S 求解器（投影法 + Boussinesq + 动态帧导出）
│   ├── coupler.py      # 交换窗口子循环、双向边界耦合、守恒审计
│   ├── verify.py       # Case A / B / C / D + 权衡曲线 + 最小周期 + 误差归因验证
...
```bash
# 运行完整验证套件（Case A/B/C/D + 权衡 + 最小周期 + 归因）→ JSON 到 stdout
...
# 逐项单元测试（26 项断言，无需 pytest）
python run_tests.py
```

## 接口约定（跨 1D-3D 的单位契约）

接口上所有量均以 **SI 基本单位**传递，派生量（流量 kg/s、压力 Pa、温度 K）都带有
标签，使两侧绝不误解量级。`units.consistent(a, b)` 是无量纲的一致性检验，用于验证两个
值是否指同一物理量。
...
| Case C | 高压高扩散能量通道变体 | 反向耦合真实参与（背压抬升 18.15 kPa） |
| Case D | 亚临界非壅塞流 | 对照文献等熵亚临界收敛喷嘴关系 |
| 权衡曲线 | 交换周期扫描 | 延迟 vs 界面误差 vs 综合评分 |
| 最小周期 / 误差归因 | 扫描 + 敏感性 | 最小可行交换周期、Case A 偏差分解 |