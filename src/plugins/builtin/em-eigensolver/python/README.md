...
**位移逆谱变换**。给定目标 `σ`，内核作用于 `Op(q) = (A − σI)⁻¹ q`，Ritz 值映射回 `λ = σ + 1/θ`。内层系统厄密**不定**（CG 不适用），故用 MINRES。内层求解**有意不精确**（`rtol = 1e-6`）：外层 Rayleigh-Ritz 重正交化恢复全部精度，而近奇异的 `A − σI` 只需有界的内层迭代数。位移逆路径的有效外层容差为 `max(tol, 20 × minres_rtol)`。注意该下限按**相对残差**判定（‖Ay − λy‖ / max|λ|，见下文"收敛认证"）：基准 JSON 归档的绝对 `max_residual` 与它**不可直接比较**，应对照 `certified_rel_residual` 字段——如 n=102400、σ=0.5 对照行，绝对 5.9e-05 ↔ 相对 7.4e-06 ≤ 2×10⁻⁵。
...
| 选 `1e-12` 容差却停在约 `2e-5` | 位移逆 Lanczos 有效容差下限 `20 × minres_rtol`（按相对残差判定，见"核心算法原理"） | 放宽预期或改用其他内核 |
